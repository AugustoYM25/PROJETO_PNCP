...
datas = pd.date_range(start="2023-07-01", end="2025-12-31")
...
for data_atual in datas:
    ...
    while pagina <= total_paginas:
        ...
        try:
            ...
            print(f"pg {pagina}/{total_paginas}: +{linhas_bloco} registros inseridos (Total acumulado: {total_inserido})")
            ...
            print(f" ERRO DE REDE/API  dia {data_formatada}, pagina {pagina}: {err_http}")
            break 
            
        except Exception as e:
            print(f"ERRO DE EXECUCAO no dia {data_formatada}, pagina {pagina}: {e}")
            break

print(f"\n Total de {total_inserido} contratos inseridos no DB_PNCP.")
